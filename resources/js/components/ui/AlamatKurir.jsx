...
export default function AlamatKurirPage() {
  ...
  if (!selection.subDistrict) {
    console.warn("Kelurahan belum dipilih. selection.subDistrict:", selection.subDistrict);
  ...
  if (!districtId) {
    console.warn("Kecamatan tidak valid. selection.district:", selection.district);
    Toast.fire({ icon: "warning", title: "Kecamatan tujuan tidak valid." });
    return;
  }

  const destinationDistrictId = String(districtId); // ✅ JAMIN STRING

  if (!cartWeight || cartWeight <= 0) {
    console.warn("Berat keranjang tidak valid:", cartWeight);
    Toast.fire({ icon: "warning", title: "Berat keranjang tidak valid." });
    return;
  }

  console.log("Mengirim request ke API dengan ", {
  ...
  try {
    ...
    const res = await fetch("/api/rajaongkir/calculate-shipping", {
      ...
      body: JSON.stringify({
        origin: FIXED_ORIGIN_ID,
        destination: destinationDistrictId, // ✅ string
        weight: cartWeight,
        courier: selectedCourier.id,
      }),
    });
    ...
    if (!res.ok) {
      ...
      console.error("Respons error dari server (raw):", text);
      let errorMessage = "Gagal menghitung ongkir.";
      try {
        const json = JSON.parse(text);
        errorMessage = json.error || json.message || errorMessage;
      } catch (parseError) {
        console.error("Gagal parse respons sebagai JSON. Ini mungkin HTML:", parseError);
    ...
    console.log("Respons sukses dari API:", data);
    if (data.results && Array.isArray(data.results)) {
      setResults(data.results);
    } else {
      console.warn("Respons tidak mengandung data ongkir yang valid:", data);
      throw new Error("Respons tidak mengandung data ongkir.");
    }
  } catch (err) {
    console.error("Error keseluruhan di handleCheckShipping:", err);
    ...
    useEffect(() => {
        if (step !== "courier" || loading) return;

        const handleKeyDown = (e) => {
            if (e.key === "ArrowDown") {
                e.preventDefault();
                setSelectedIndex((prev) => (prev + 1) % COURIERS.length);
            } else if (e.key === "ArrowUp") {
                e.preventDefault();
                setSelectedIndex((prev) => (prev - 1 + COURIERS.length) % COURIERS.length);
            } else if (e.key === "Enter") {
                e.preventDefault();
                setSelectedCourier(COURIERS[selectedIndex]);
                handleCheckShipping();
            }
        };
    ...
    }, [step, selectedIndex, loading, handleCheckShipping]);

    useEffect(() => {
        const timer = setTimeout(async () => {
            if (searchQuery.length < 3 || !selection.province) {
                setSuggestions([]);
                return;
            }
    ...
    }, [searchQuery, selection.province, selection.city, selection.district]);
    ...
    const handleConfirm = () => {
        if (!selection.subDistrict || results.length === 0) {
            Toast.fire({ icon: "warning", title: "Pilih alamat dan pastikan hasil tersedia." });
            return;
        }

        const selectedResult = results[0];
        if (!selectedResult || selectedResult.services.length === 0) {
            Toast.fire({ icon: "warning", title: "Hasil kurir tidak valid." });
            return;
        }

        const selectedService = selectedResult.services[0];
        const fullAddress = `${selection.subDistrict.name}, ${selection.district.name}, ${selection.city.name}, ${selection.province.name}`;
        localStorage.setItem('alamat_lengkap', fullAddress);
        localStorage.setItem('shipping_cost', selectedService.cost.toString());
        localStorage.setItem('selected_courier_code', selectedCourier.id);
        localStorage.setItem('selected_courier_name', selectedResult.courier);
    ...
    const selectedCourierName = selectedCourier.name;
        ...
        <div className="min-h-screen bg-[#f4d03f] flex flex-col font-sans overflow-hidden">
            ...
            <main className="flex-1 relative flex flex-col px-8 overflow-hidden">
                <div className="flex gap-4 mb-6 z-10">
                    {["address", "courier", "results"].map((s, idx) => (
                        <div
                            key={s}
                            className={cn(
                    ...
                    <div className="flex-1 overflow-y-auto pr-4 pb-20 scrollbar-hide">
                                ...
                                <div className="bg-white border-4 border-black p-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                                    ...
                                    <div className="space-y-6">
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            {selection.province && (
                                                <span className="bg-black text-white px-3 py-1 text-xs font-black italic uppercase skew-x-[-10deg]">
                                            ...
                                            {selection.subDistrict && (
                                                <span className="bg-[#f4d03f] text-black px-3 py-1 text-xs font-black italic uppercase skew-x-[-10deg] border-2 border-black">
                        ...
                        {step === "courier" && (
                            <div className="animate-in fade-in slide-in-from-bottom-8 duration-500">
                                <div className="bg-white border-4 border-black p-8  shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                                    <h2 className="text-3xl font-black italic uppercase mb-8 text-black">Pilih Kurir</h2>
                                        ...
                                        {COURIERS.map((c, idx) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={cn(
                                                        "w-full text-left py-4 px-6 flex items-center justify-between transition-all group",
                                                        isSelected
                                                            ? "bg-black text-white scale-105 shadow-[8px_8px_0px_0px_#f4d03f]"
                                                            : "bg-white text-black hover:bg-black/5",
                                                    )}
                                                >
                                                    <span className="text-2xl font-black italic uppercase tracking-wider">{c.name}</span>
                                                    <ChevronRight
                                                        className={cn(
                        ...
                        {step === "results" && (
                            <div className="animate-in fade-in zoom-in-95 duration-500 space-y-6">
                                {loading ? (
                                    <div className="bg-white border-4 border-black p-20 flex flex-col items-center justify-center gap-6 ">
                                    ...
                                    <div className="bg-red-500 border-4 border-black p-8 flex items-center gap-6  text-white">
                                    ...
                                    <div className="space-y-4">
                                        <div className="bg-black text-white p-4 mb-8 shadow-[4px_4px_0px_0px_#f4d03f]">
                                            <h3 className="text-xl font-black italic uppercase tracking-widest">
                                                Hasil Ditemukan: {results.length} Kurir
                                            </h3>
                                        </div>
                                        {results.map((res, i) => (
                                            <div
                                                key={i}
                                                className="animate-in fade-in slide-in-from-left-8"
                                                style={{ animationDelay: `${i * 100}ms` }}
                                            >
                                                <div className="bg-white border-4 border-black p-6 flex flex-col gap-4 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden group">
                                                    ...
                                                    {res.services.map((s, idx) => (
                                                        ...
                                                        >
                                                            <div>
                                                                <h4 className="text-xl font-black italic uppercase text-black">{s.service}</h4>
                                                                <p className="text-sm font-bold text-black/60 uppercase">{s.description || "-"}</p>
                                                            </div>
                                                            <div className="text-right">
                                                                <p className="text-2xl font-black italic text-black">Rp {s.cost.toLocaleString()}</p>
                    ...
                    <aside className="w-80 hidden lg:flex flex-col gap-6 animate-in fade-in slide-in-from-right-12 duration-700">
                        <div className="flex-1 bg-black text-white p-6 flex flex-col relative overflow-hidden">
                            ...
                            <div className="space-y-4 relative z-10">
                                ...
                                <div className="flex flex-col">
                                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] opacity-50">Kurir</span>
                                    <span className="text-sm font-black italic uppercase">{selectedCourierName}</span>
                                </div>
                            </div>
