...
export default function BukuPromo() {
    ...
    useEffect(() => {
        const fetchPromoBooks = async () => {
            try {
                const res = await fetch("/api/books/recommended?type=promo&limit=15");
                if (res.ok) {
                    const data = await res.json();
                    setPromoBooks(data);
                }
            } catch (error) {
                console.error("Gagal memuat buku promo:", error);
            } finally {
                setLoading(false);
            }
        };
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    {promoBooks.map((book) => {
                        ...
                        return (
                            <div
                                key={book.id}
                                className="group relative bg-white rounded-lg md:rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border border-gray-200 h-full flex flex-col"
                            >
                                {book.promo_name && (
